...
export default function SnapshotOverview({ summary, isActive }) {
  const title = (
    <div className="flex relative items-center">
      <div className="flex-1 h-[32px]">{summary.label}</div>
    </div>
  );
    ...
    <ComponentCard title={title} className={`${defaultClass} ${isActiveClass}`}>
      <>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-7 2xl:gap-x-32">
          <CopyableText text={summary.snapshotId}>
            <div>
